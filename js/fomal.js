...
$.ajax({
  ...
  data: {
    key: 'DKRBZ-ZIL6U-QEPVE-GZHJL-7CGTS-6SFZQ',  
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
/*function showWelcome() {
  ...
  let dist = getDistance(113.34499552, 23.15537143, 113.34499552, 23.15537143); //这里换成自己的经纬度
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Fomalhaut🥝 %c 你正在访问Elivagar",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Elivagar") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Elivagar站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
